...
def make_low_rank(args, model, tokenizer, device=torch.device("cuda:0"),  prune_data_pos='wikitext', prune_data_neg='wikitext'):
    """
    ...
    dataloader_pos, _ = get_loaders(prune_data_pos, nsamples=args.nsamples,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer, disentangle=args.disentangle)
    dataloader_neg, _ = get_loaders(prune_data_neg, nsamples=args.nsamples,seed=args.seed,seqlen=model.seqlen,tokenizer=tokenizer, disentangle=args.disentangle)
    print("dataset loading complete")

    num_hidden_layers = model.config.num_hidden_layers

    for layer in range(num_hidden_layers):
        layer_filter_fn = lambda x: f'layers.{layer}.' in x ### TODO # hack for llama series
        ...
        for name, module in model.named_modules():
            if layer_filter_fn(name) and isinstance(module, ActLinear):
                ...
                activation_norms_p = torch.cat(activation_norms_pos[name], dim=0).to(device) # size * d_in
                score_p = activation_norms_p @ module.base.weight.data.T # (size * d_in) @ (d_out * d_in).T --> (size, d_out)
                _ , _ ,Vp = torch.svd_lowrank(score_p.float(), q=total_rank - args.rank_pos, niter=args.niter) # (size, r) (r) (d_out, r)
                Vp_proj = (Vp @ Vp.T).type(module.base.weight.data.dtype) # (d_out, d_out)

                activation_norms_n = torch.cat(activation_norms_neg[name], dim=0).to(device) # size * d_in
                score_n = activation_norms_n @ module.base.weight.data.T # (size * d_in) @ (d_out * d_in).T --> (size, d_out)
                _ , _ ,Vn = torch.svd_lowrank(score_n.float(), q= total_rank - args.rank_neg, niter=args.niter) # (size, r) (r) (d_out, r)
                Vn_proj = (Vn @ Vn.T).type(module.base.weight.data.dtype) # (d_out, d_out)

                Vp_proj_ortho = (torch.eye(d_out, device=device) - Vp_proj).type(module.base.weight.data.dtype) # (d_out, d_out)

                module.base.weight.data.sub_(Vp_proj_ortho @ (Vn_proj @ module.base.weight.data))
...
if __name__ == '__main__':
    ...
    data_choices=["wikitext", "alpaca", "alpaca_cleaned", "alpaca_cleaned_no_safety", "align", "align_short", "misalign", "align_misalign", "misalign_align", "align_short_misalign", "none"]
    parser.add_argument("--prune_data_pos", type=str, choices=data_choices, default='misalign')
    parser.add_argument("--prune_data_neg", type=str, choices=data_choices, default='align')

    parser.add_argument("--disentangle", action="store_true", help="whether to disentangle the prompt and response when computing the wanda score")
    ...
    if True:
        ...
        device = 'cuda'
        make_low_rank(args, model, tokenizer,  device,  prune_data_pos=args.prune_data_pos, prune_data_neg = args.prune_data_neg)
        ...
        if True:
            ...
            if not os.path.exists(save_filepath):
                with open(save_filepath, "w") as f:
                    ...
                    print(f"{args.rank_pos}_{args.rank_neg}\t{args.alpha}\tPPL\t{ppl_test:.4f}", file=f, flush=True)
            else:
        ...
        if args.eval_attack:
            ...
            if True:
                ...
                for include_inst in [True, False]:
                    ...
                    with open(save_filepath, "a") as f:
                        print(f"{args.rank_pos}_{args.rank_neg}\t{suffix}\tASR_basic\t{score:.4f}", file=f, flush=True)
                    ...
                    with open(save_filepath, "a") as f:
                        print(f"{args.rank_pos}_{args.rank_neg}\t{suffix}\tASR_basic_nosys\t{score:.4f}", file=f, flush=True)
                    ...
                    with open(save_filepath, "a") as f:
                        print(f"{args.rank_pos}_{args.rank_neg}\t{suffix}\tASR_multiple_nosys\t{score:.4f}", file=f, flush=True)
                ...
                with open(save_filepath, "a") as f:
                    print(f"{args.rank_pos}_{args.rank_neg}\t{suffix}\tASR_gcg\t{score:.4f}", file=f, flush=True)
                del vllm_model   
        ...
        if args.eval_zero_shot:
           ...
           num_shot = 0
           results = eval_zero_shot(modeltype2path[args.model], model, tokenizer, task_list, num_shot, accelerate, limit=1000)
           print("********************************")
           print("zero_shot evaluation results")
           sum_acc = 0
           with open(save_filepath, "a") as f:
               for k,v in results['results'].items():
                   print(f"{args.rank_pos}_{args.rank_neg}\t{args.alpha}\t{k}\t{v['acc']:.4f}", file=f, flush=True)
                   sum_acc += v['acc']
               print(f"{args.rank_pos}_{args.rank_neg}\t{args.alpha}\taveraged\t{sum_acc/len(task_list):.4f}", file=f, flush=True)
