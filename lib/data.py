...
def get_alpaca(nsamples, seed, seqlen, tokenizer, disentangle=False, dataset='alpaca'):
    ...
    if disentangle:
        ...
        for i in range(nsamples):
            trainenc_prompt = tokenizer(traindata_sampled['prompt'][i], return_tensors='pt')
            trainenc_response = tokenizer(traindata_sampled['response'][i], return_tensors='pt')
            inp = torch.cat((trainenc_prompt.input_ids, trainenc_response.input_ids[:, 1:]), dim=1) # to remove the first token of the response ('1')
            tar = inp.clone()
            trainenc_prompt_len = trainenc_prompt.input_ids.shape[1]
            tar[:, :trainenc_prompt_len] = -100
            trainloader.append((inp, tar))
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, tokenizer=None, disentangle=False):
    if name == 'wikitext':
    ...
    if name == "align_short":
        return get_align(nsamples, seed, seqlen, tokenizer, disentangle=disentangle, mode='short')
