...
def prune_wanda_v2(args, model, tokenizer, model_base=None, device=torch.device("cuda:0"), prune_n=0, prune_m=0, prune_data='wikitext'):
...
def prune_wandg_v1(args, model, tokenizer, model_base=None, device=torch.device("cuda:0"), prune_n=0, prune_m=0, prune_data='wikitext'):
    model = make_Act(model, verbose=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--prune_data", type=str, choices=["wikitext", "align", "align_llama2-7b-chat", "misalign"], default='wikitext')
    ...
    if True:
        ...
        prune_wandg(args, chat, tokenizer, model_base, device, prune_n=prune_n, prune_m=prune_m, prune_data=args.prune_data)
